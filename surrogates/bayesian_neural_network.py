...
class BayesianNeuralNetwork_Medium(BatchedMultiOutputGPyTorchModel):
    """Medium yesian Neural Network (BNN) surrogate class. """

    def __init__(self, parameters: Parameters, dataset: Dataset, name: str = "BNN_Medium"):
        super().__init__()
        self.name = name
        self.d = parameters.d
        self.std_change = parameters.std_change
        ...
        self.model = nn.Sequential(
            ...
            # nn.ReLU(),
            bnn.BayesLinear(
                prior_mu=0.0, prior_sigma=1.0, in_features=self.d, out_features=n_z_1,
            ),
            nn.ReLU(),
            bnn.BayesLinear(
                prior_mu=0.0,
        ...
        self.kl_weight = 1.0
        self._set_dimensions(train_X=dataset.data.X_train, train_Y=dataset.data.y_train)
        self.optimizer = optim.Adam(self.model.parameters(), lr=4 * 1e-3)
        self.fit(X_train=dataset.data.X_train, y_train=dataset.data.y_train)
