...
class FashionMNIST(object):
    ...
    def sample_initial_dataset(self) -> None:
        self.X_full_origin = np.load("./datasets/FashionMNIST/optim_dataset/hyperparams.npy")
        self.y_full_origin = -np.load("./datasets/FashionMNIST/optim_dataset/accuracies.npy")
        # self.y_test = np.load("./datasets/MNIST/optim_dataset/losses.npy")

        self.X_full = (
            self.X_full_origin[:, np.newaxis] if self.X_full.ndim == 1 else self.X_full
        )
        self.y_full = (
            self.y_full_origin[:, np.newaxis] if self.y_full.ndim == 1 else self.y_full
        )
